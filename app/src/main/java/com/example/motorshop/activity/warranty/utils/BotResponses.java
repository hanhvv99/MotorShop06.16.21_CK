...
//            String equation = mess.substring(Integer.parseInt("solve"));
            ...
            try {
                int answer = com.example.motorshop.activity.warranty.utils.SolveMath.solveMath(equation);
        ...
        if(mess.contains("time") && mess.contains("?")){
            com.example.motorshop.activity.warranty.utils.Time time = new com.example.motorshop.activity.warranty.utils.Time();
            mess = time.timeStamp();

        }
        if(mess.contains("mở google")||mess.contains("google")){
            mess = com.example.motorshop.activity.warranty.utils.Constants.OPEN_GOOGLE;
        }
